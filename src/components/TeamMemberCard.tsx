...
const TeamMemberCard = ({ member, onClick }: TeamMemberProps) => {
  ...
  return (
    <>
      ...
      >
        <div 
          className="flex flex-col items-center justify-between relative rounded-3xl p-4 h-full overflow-hidden border border-blue-100 bg-white/80 backdrop-blur-xl shadow-xl group-hover:shadow-2xl group-active:scale-95 transition-all duration-300"
        >
          {/* Bordo animato elegante */}
          <span className="border-animated-shine absolute inset-0 z-0" />
          {/* Alone colorato animato dietro la foto */}
          <span className="absolute top-4 left-1/2 -translate-x-1/2 w-20 h-20 rounded-full bg-gradient-to-br from-blue-400/30 via-blue-400/20 to-blue-400/10 blur-2xl animate-orb z-0" />
          ...
          <div className="relative z-10 w-full flex flex-col items-center justify-between h-full">
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="relative mb-3">
                <div className="w-14 h-14 bg-gradient-to-br from-gray-400 to-gray-600 rounded-full flex items-center justify-center overflow-hidden border-3 border-blue-200 group-hover:border-blue-300 transition-all duration-300 shadow-md">
                  <img
                ...
                <div className="absolute -bottom-1 -right-1 bg-white rounded-full border border-blue-200 shadow p-1">
            ...
            <div className="flex flex-wrap justify-center gap-1 mb-2">
              {member.skills.slice(0, 3).map((skill, skillIndex) => (
                <span
                  key={skillIndex}
                  className="px-2 py-1 bg-blue-100 border border-blue-200 rounded-full text-blue-700 text-xs"
                >
                  {skill}
                </span>
              ))}
      ...
      <div className="hidden md:block group relative">
        <div className="relative bg-white/80 backdrop-blur-xl border border-blue-200 rounded-3xl p-6 h-full overflow-hidden group-hover:border-blue-300 group-hover:shadow-lg transition-all duration-500">
          ...
          >
            <div className="w-full h-full bg-gradient-to-br from-gray-400 to-gray-600 rounded-full flex items-center justify-center overflow-hidden border-4 border-blue-200 group-hover:border-blue-300 transition-all duration-300">
              <img
                src={getImagePath(member.name)}
                alt={member.name}
                className="w-full h-full object-cover"
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {member.skills.map((skill, skillIndex) => (
                <motion.span
                  ...
                  whileHover={{ scale: 1.05 }}
                  transition={{ duration: 0.2 }}
                  className="px-3 py-1 bg-blue-100 border border-blue-200 rounded-full text-blue-700 text-sm hover:bg-blue-200 transition-all duration-300"
                >
                  {skill}
                </motion.span>
              ))}
